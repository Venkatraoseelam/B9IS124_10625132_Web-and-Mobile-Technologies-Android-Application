...
public class ViewUserAdmin extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    TextView username_tv, name_tv, email_tv, n_bikes_tv, nodata;
    ...
    public void getData() {
        ...
        Cursor result2 = db.getAllData_User_bike();

        if (result.getCount() == 0) {
            hasData = false;
        }
        else {
            HashMap<String,String> n_bikes = new HashMap<String, String>();
            while(result2.moveToNext()){
                n_bikes.put(result2.getString(0),result2.getString(1));
            }
            bf = new StringBuffer();
            while (result.moveToNext()) {
                ...
                bf.append(n_bikes.get(result.getString(0)) + "\n");
            }
    ...
    public void addData() {
        ...
        for (int i = 0; i < data_row.length; i++)
        {
            ...
            n_bikes_tv = new TextView(this);
            n_bikes_tv.setText(data[8]);
            n_bikes_tv.setTextColor(getResources().getColor(R.color.colorAccent,null));
            n_bikes_tv.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
            //price_tv.setBackgroundColor(Color.LTGRAY);
            n_bikes_tv.setTextSize(15);
            n_bikes_tv.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,LayoutParams.WRAP_CONTENT));
            n_bikes_tv.setPadding(15, 20, 15, 20);
            n_bikes_tv.setTypeface(Typeface.SERIF, Typeface.NORMAL);
            tableRow.addView(n_bikes_tv);
            ...
            tableRow.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    buffer.append("Address: "+data[4] +", "+data[5]+ "\n\n");
...
//                    }
                    buffer.append("Number of Bikes: "+data[8] + "\n\n");

                    Cursor result_bike = db.getRegNo_bike_username(data[0]);
                    if (result_bike.getCount() != 0) {
                        buffer.append("Bike Numbers: ");
                        result_bike.moveToNext();
                        buffer.append(result_bike.getString(0));
                        while(result_bike.moveToNext()){
                            buffer.append(", "+result_bike.getString(0));
                        }
                        buffer.append("\n\n");
                    }
    ...
    private void showMessage(String title, String message, final String username, final String n_bikes) {
                ...
                builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        if(res.getCount() > 0){
                            ...
                            Integer delRows = db.deleteData_bike_username(username);
                            if(delRows > 0){
